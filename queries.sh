...
echo "$($PSQL "SELECT ROUND(AVG(winner_goals), 2) AS rounded_average FROM games")"
...
echo "$($PSQL "SELECT teams.name, games.year FROM teams INNER JOIN games ON teams.team_id = games.winner_id WHERE games.round='Final'")"
